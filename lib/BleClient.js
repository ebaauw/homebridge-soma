...
class BleClient extends SafeEventEmitter {
  ...
  constructor (params = {}) {
    ...
    this.noble = new Noble(new NobleBindings({ userChannel: true }))
    this.noble
      ...
      .on('stateChange', async (newState) => {
        ...
        try {
          ...
          if (this.enabled) {
            this._config.address = this.noble.address.toUpperCase()
  ...
  async checkPlatform () {
    if (this._config == null) {
      ...
      if (this._config.platform === 'darwin') {
        this._config.platform = 'macOS'
        this.macOs = true
      } else {
        this._config.adapter = 'hci' + (process.env.NOBLE_HCI_DEVICE_ID || '0')
      }
      if (
        this._config.platform === 'linux' &&
        ['arm', 'arm64'].includes(this._config.arch)
      ) {
        try {
          await fs.access('/usr/bin/vcgencmd')
          this._config.platform = 'Raspberry Pi OS'
          this.piOs = true
          this._config.supported = true
        } catch (error) {}
      }
    }
  }
  ...
  async search (duration = this.scanDuration, allowDuplicates = this.allowDuplicates) {
    ...
    return this.request(
      ...
      this.noble.startScanningAsync([], allowDuplicates)
    )
  }
  ...
  reset () {
    return this.request(
      ...
      new Promise((resolve, reject) => { resolve(this.noble.reset()) })
    )
  }
...
class BlePeripheralDelegate extends SafeEventEmitter {
  ...
  async connect (duration = this.connectionDuration, reconnectOnInitialConnect = true) {
    if (this.connectionTimer != null) {
      clearTimeout(this.connectionTimer)
      delete this.connectionTimer
    }
    ...
    if (this.serviceDelegates == null) {
      try {
        ...
        if (reconnectOnInitialConnect) {
          await this.disconnect()
          return this.connect(duration)
        }
      } catch (error) {
...
class BleServiceDelegate {
  ...
  async discover () {
    ...
    throw new BleError('cannot discover service', request)
  }
...
class BleCharacteristicDelegate {
  constructor (serviceDelegate, characteristic) {
  ...
  async discover () {
    ...
    throw new BleError('cannot discover characteristic', request)
  }
