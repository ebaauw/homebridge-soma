...
class SomaAccessory extends homebridgeLib.AccessoryDelegate {
  ...
  async heartbeat (beat) {
    try {
      ...
      const { motorSpeed, localTimeOffset, bootSeq } = await this.client.getShadeConfig()
      ...
      if (this.rebooted) {
        ...
        if (this.values.firmware !== packageJson.engines.soma) {
          this.warn(
      ...
      if (now.toDateString() !== this.today) {
        ...
        this.lightSensorService.values.sunrise = sunriseDate.toString().slice(0, 21)
        this.lightSensorService.values.sunset = sunsetDate.toString().slice(0, 21)
