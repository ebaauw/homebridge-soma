...
class SomaPeripheral extends BleClient.BlePeripheralDelegate {
  ...
  async _setMotorControl (command) {
    await this.subscribe('motorService', 'motorControl')
    await this.write(
      'motorService', 'motorControl', Buffer.from([command])
    )
    return (await this.notification(
      'motorService', 'motorControl'
    )).result[0].parsedValue
  }

  async up () {
    return this._setMotorControl(motorTriggerCommands.up)
  }

  async down () {
    return this._setMotorControl(motorTriggerCommands.down)
  }

  async stop () {
    return this._setMotorControl(motorTriggerCommands.stop)
  }

  async stepUp () {
    return this._setMotorControl(motorTriggerCommands.stepUp)
  }

  async stepDown () {
    return this._setMotorControl(motorTriggerCommands.stepDown)
  }

  async stopAtNextStep () {
    return this._setMotorControl(motorTriggerCommands.stopAtNextStep)
  }
  ...
  async getShadeConfig () {
    return this._getShadeConfig([
      shadeConfigCommands.motorSpeed, shadeConfigCommands.bootSeq
    ])
  }
