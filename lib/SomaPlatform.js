...
class SomaPlatform extends homebridgeLib.Platform {
  constructor (log, configJson, homebridge) {
    ...
    optionParser
      ...
      .stringKey('name')
      .arrayKey('shades')
      .intKey('resetInterval', 5, 120) // minutes
      .intKey('rssi', -100, -50)
    ...
    try {
      ...
      this.config.resetInterval *= 60 // minutes -> seconds
  ...
  async init (beat) {
    ...
    this.client
      ...
      .on('enabled', (config) => {
        this.log('running on %s on %s', config.platform, config.arch)
        if (!config.supported) {
  ...
  async heartbeat (beat) {
    ...
    if (beat % this.config.resetInterval === this.config.resetInterval - 1) {
      try {
