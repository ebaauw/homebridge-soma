      ...
      .on('shadeFound', async (device) => {
        if (this.shades[device.id] == null) {
          ...
          try {
            ...
            this.debug('%s: %j', device.data.displayName, map)
            await delegate.disconnect()
            this.shades[device.id] = new SomaAccessory(this, {
              id: map.shadeService.shadeMacAddress.replace(/-/g, ''),
              deviceId: device.id,
              name: map.shadeService.shadeName,
              model: map.deviceInformation.hardwareRevision,
              firmware: map.deviceInformation.softwareRevision.slice(1),
              supportsUp: map.motorService.motorCalibration.venetianMode
            })
          } catch (error) {
